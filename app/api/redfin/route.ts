...
export async function GET(req: Request) {
  ...
  try {
    const htmlRes = await fetch(target, {
      headers: { "User-Agent": "Mozilla/5.0 (house-dashboard)" },
    });
    ...
    const out: any = {
      ...
      livingAreaSqft: null,
      lotSizeSqft: null,
      schools: [] as { name: string; rating?: number; level?: string }[],
    };
    ...
    const lotMatch = text.match(/Lot\s*Size[^\d]*(\d{3,6})\s*(?:sq\.?\s*ft|square\s*feet)/i);
    if (lotMatch && !out.lotSizeSqft) out.lotSizeSqft = Number(lotMatch[1]);

    const areaMatch =
      text.match(/Living\s*Area[^\d]*(\d{3,6})\s*(?:sq\.?\s*ft|square\s*feet)/i) ||
      text.match(/Square\s*Feet[^\d]*(\d{3,6})/i);
    if (areaMatch && !out.livingAreaSqft) out.livingAreaSqft = Number(areaMatch[1]);

    const faceMatch = text.match(
      /(?:Faces|Facing|Direction\s*Faces?)\s*[:\-]?\s*(North(?:east)?|South(?:east)?|East|West|South(?:west)?|North(?:west)?)/i
    );
    ...
    // 3) Assigned schools (+ GreatSchools rating when visible)
    ...
    out.schools = schoolBlocks.filter((s) => {
